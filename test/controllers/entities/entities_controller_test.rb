...
class Entities::EntitiesControllerTest < ActionController::TestCase
  ...
  test 'GET index' do
    get :index, entity_class_name: Entity::Test.name.pluralize
  ...
  test 'GET index with filters' do
    get :index, entity_class_name: Entity::Test.name.pluralize, filters: { name: 'test' }
    assert_response :success
    assert assigns[:entities].all.size > 1
    assert_equal 1, assigns[:page]
  end
  ...
  test 'GET index with page' do
    get :index, entity_class_name: Entity::Test.name.pluralize, page: 2
    assert_response :success
    assert assigns[:entities].all.size > 1
  ...
  test 'GET show' do
    get :show, entity_class_name: Entity::Test.name.pluralize, id: '219A7C20-58B8-11E5-B850-6BAC9D6E46F5'
    assert_response :success
