class EntityTypesController < TypesController
  ...
  def index
    ...
    @entity_types = type_class.search page: @page, per_page: @per 
  end
  ...
  def measurement_types
    ...
    render :json => @measurement_types.to_json(:only => [:id, :related_entity_type_id, :name, :description, :frequency, :perm_link, :unit])
  end
